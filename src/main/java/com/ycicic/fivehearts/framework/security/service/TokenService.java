...
import com.ycicic.fivehearts.common.utils.uuid.IdUtils;
...
public class TokenService {
    ...
    private static final Long MILLIS_MINUTE_TEN = 3 * 24 * 60 * 60 * 1000L;
        ...
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                if (StringUtils.isEmpty(uuid)) {
                    uuid = (String) claims.get(Constants.LOGIN_APP_USER_KEY);
                }
     ...
     * @return 令牌
     */
    public String createAppToken(LoginUser loginUser) {
        String token = IdUtils.fastUUID();
        loginUser.setToken(token);
        setUserAgent(loginUser);
        refreshToken(loginUser);
